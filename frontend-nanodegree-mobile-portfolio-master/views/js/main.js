...
var resizePizzas = function(size) {
  ...
  // Declared the pizza length and pizza container outside the loop.
  function changePizzaSizes(size) {
  var randomPizzaLength = document.getElementsByClassName("randomPizzaContainer").length;
  ...
  var newwidth = [];
    ...
    for (var i = 0; i < randomPizzaLength; i++) {
      var dx = determineDx(randomPizzacont[i], size);
		newwidth[i] = (randomPizzacont[i].offsetWidth + dx) + 'px';
    }
    for (i = 0; i < randomPizzaLength; i++) {
	randomPizzacont[i].style.width = newwidth[i];
	    }
...
// declared the pizzaDiv Outside the loop and called it
...
//declared scrollTop and Phase outside the loop
